...
You must supply the fixed point. If you do not, the skill asks for one rather than guessing; it then checks the ref resolves and the diff is non-empty before either pass starts, so a typo'd branch name fails in front of you immediately.
...
Not without checking. Review output is a hypothesis, not evidence: one team reported a dozen breaking changes that prose-based reviews had waved through. The skill presents the two reports verbatim or lightly cleaned rather than re-verifying each claim against the files, so a finding can cite the wrong location or overstate an impact. Read the citation on each finding before acting on it. That every finding is required to carry one (a standards rule, a smell plus its hunk, or a spec line) is what makes this checkable at all.
...
- It refuses to start on a bad ref or an empty diff, before either pass begins.
